...
import com.efeiyi.ec.security.auth.UserDetailsService;
...
public class SigninController extends BaseController {
    ...
    @ResponseBody
    public MyUser getCurrentUser(HttpServletRequest request) {
        return AuthorizationUtil.getMyUser(request);
    }
    ...
    public String login(HttpServletRequest request) {
        ...
        String result;
        try {
            abstractSecurity.authenticateByPassword(request, username, password);
        } catch (AuthenticationException ae) {
            result = "{\"login\":\"false\",\"message\":\"" + ae.getMessage() + "\"}";
            return result;
        }


        String redirectUrl = abstractSecurity.getSavedRequest(request);
        if (redirectUrl != null) {
            result = "{\"login\":\"true\",\"redirect\":\"" + redirectUrl + "\"}";
        } else {
    ...
    public String updatePassword(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if ("checked".equals(request.getSession().getAttribute(username))) {
            ...
            User myUser = (User) userManager.getUserByUsername(username);
            myUser.setPassword(StringUtil.encodePassword(password, "sha"));
            baseManager.saveOrUpdate(User.class.getName(), myUser);
